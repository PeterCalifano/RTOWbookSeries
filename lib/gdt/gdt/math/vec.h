...
#include <algorithm>
#include <iostream>
#include <lib/gdt/gdt/gdt.h>
#include <lib/gdt/gdt/math/constants.h>
#include <math.h>
...
namespace gdt
{
  ...
  inline __both__ T dot(const vec_t<T, size> &a, const vec_t<T, size> &b)
  {
    T dotproduct = 0;
   ...
   */
  template <typename T, int size>
  inline __both__ vec_t<T, size> normalize(const vec_t<T, size> &v)
  {
    return v * 1.f / gdt::overloaded::sqrt(dot<T, size>(v, v));
  }
   ...
   */
  template <typename T, int size>
  inline __both__ T length(const vec_t<T, size> &v)
  {
